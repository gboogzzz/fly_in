...
class Router:
    @staticmethod
    def route(graph: Graph, start: Zone, end: Zone, nb_drones: int) -> Dict[Drone, List[Zone]]:
        paths = Router._find_sufficient_paths(graph, start, end, nb_drones)
        distribuition = Router._distribute_drones(graph, paths, nb_drones)
        result: Dict[Drone, List[Zone]] = {}
        drone_id = 1

        for index, count, in distribuition.items():
            path = paths[index]
            for _ in range(count):
                drone = Drone(drone_id=drone_id, current_zone=start, path=path)
                result[drone] = path
                drone_id += 1
    ...
    @staticmethod
    def _find_sufficient_paths(graph: Graph, start: Zone, end: Zone, nb_drones: int) -> List[List[Zone]]:
        k = 1
        ...
        while total_capacity < nb_drones:
            ...
            if len(new_paths) == len(paths):
                #KPaths didnt find any new path
                break
            paths = new_paths
            capacities = [Router._path_capacity(graph, path) for path in paths]
            total_capacity = sum(capacities)
    ...
    @staticmethod
    def _distribute_drones(graph: Graph, paths: List[List[Zone]], nb_drones: int) -> Dict[int, int]:
